...
- インストール: 公式オーナーズストア (= 審査中) または release archive アップロード
- 対応経路: **uniple checkout (= JPYC ウォレット直結)** ✅、 LINE 経路は MVP 後

## 2.2 EC-CUBE 2.x (= 2.17.x)
...
- 動作 PHP: **7.0+ 必須** (= `random_bytes()` 等 PHP 7.0+ 関数を使用、 PHP 5.6 はサポート対象外)
- 公式 install: オーナーズストア > プラグイン管理 経由でインストール (= dtb_plugin row 自動生成、 設定画面アクセス可)
