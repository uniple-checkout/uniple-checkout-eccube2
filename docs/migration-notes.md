...
|---|---|---|---|
| **Phase 1** | 2026-05-10 ~ 2026-05-12 | docs / 案内文 SSOT 化、 内部 `?wc=1` 付与は legacy 互換で残置 | MerchantSite.checkoutMode + lineUsageEnabled + environment + AuditLog 追加 + Hosted Checkout routing 拡張 |
| **Phase 2** | **2026-05-12 完了 ✅** (= D user 実 wallet e2e smoke 前倒し) | plugin 内部 `?wc=1` 付与ロジック削除 (= 経路振り分けは uniple SSR で完結、 r22 設計訂正) | `/checkout/{sessionId}` SSR で `MerchantSite.checkoutMode` 経由 routing (= step 4-7 完成) |
...
## 4. `?wc=1` query の自動付与について — Phase 2 (= 2026-05-12) で削除完了 ✅

旧記載 (= Phase 1 期間中): plugin が `?wc=1` query を自動付与していました。

**2026-05-12 Phase 2 commit (= r22 設計訂正)** で `?wc=1` 付与ロジックを **完全削除**:
- plugin = `POST /api/merchant/checkout/sessions` 呼出後、 返却 `checkoutUrl` をそのまま redirect (= thin client)
- 経路振り分け (= LINE / WC / both) は uniple `/checkout/{sessionId}` SSR で `MerchantSite.checkoutMode` 経由完結
- 加盟店側の追加設定は不要、 plugin update で自動切替
